  ...
  <div class="container">
    <div class="profile-page">
      ...
      <div class="card" style="margin-top:16px">
        ...
        <p v-if="profileMsg" :class="profileMsg.includes('成功') ? 'success-msg' : 'error'">{{ profileMsg }}</p>
      ...
      <div class="card" style="margin-top:16px">
        ...
        <p v-if="pwMsg" :class="pwMsg.includes('成功') ? 'success-msg' : 'error'">{{ pwMsg }}</p>
...
const initials = computed(() => {
  ...
  return (l.charAt(0) + f.charAt(0)).toUpperCase() || '智'
})
...
.avatar-placeholder {
  width: 100%; height: 100%; background: var(--color-primary-soft);
  color: var(--color-primary); display: flex; align-items: center; justify-content: center;
  font-size: 24px; font-weight: 700;
}
.avatar-overlay {
  position: absolute; inset: 0; background: rgba(15, 23, 42, 0.54);
  display: flex; align-items: center; justify-content: center;
  color: white; font-size: 13px; font-weight: 700; opacity: 0; transition: opacity 160ms var(--ease-out);
}
...
.success-msg { color: var(--color-success); font-size: 13px; margin-top: 8px; }
</style>
