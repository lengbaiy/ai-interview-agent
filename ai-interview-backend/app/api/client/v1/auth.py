...
@router.post(
    ...
    dependencies=[Depends(rate_limit("auth-register", limit=5, window_seconds=60))],
)
async def register(
...
@router.post(
    ...
    dependencies=[Depends(rate_limit("auth-verification", limit=3, window_seconds=60))],
)
...
@router.post(
    ...
    dependencies=[Depends(rate_limit("auth-password-reset", limit=3, window_seconds=60))],
)
...
async def upload_avatar(
    ...
    if not ext:
        return ApiResponse.error(message="仅支持 PNG、JPEG、GIF 或 WebP 图片")
...
async def change_password(
    ...
    client_auth_service.validate_password(data.new_password)
    ...
    await db.execute(
        update(Token).where(Token.user_id == current_user.id, Token.is_active).values(is_active=False)
    )
    await db.commit()
