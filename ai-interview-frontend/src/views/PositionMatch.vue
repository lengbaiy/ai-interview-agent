<template>
  <div class="container">
    ...
    <div v-if="result" class="result-area">
        ...
        <div class="position-list">
          ...
          >
            ...
            <div class="position-actions">
              <router-link class="btn-secondary position-jobs-link" :to="{ path: '/jobs', query: { search: pos.title } }">
                查看真实职位
              </router-link>
              <button
                class="btn-primary"
      ...
      <details class="card process-card" v-if="intermediateSteps?.length">
        <summary>
          <span class="process-summary-title">本次分析过程</span>
          <span class="process-summary-meta">{{ intermediateSteps.length }} 个步骤已完成</span>
        </summary>
        <div class="process-list">
          <article v-for="(step, i) in intermediateSteps" :key="i" class="process-step">
            <div class="process-index">{{ i + 1 }}</div>
            <div class="process-content">
              <div class="process-title-row">
                <h3>{{ step.title || step.tool }}</h3>
                <span class="process-status">已完成</span>
              </div>
              <p>{{ step.summary || '该步骤已完成。' }}</p>
              <div v-if="step.highlights?.length" class="process-highlights">
                <span v-for="highlight in step.highlights" :key="highlight">{{ highlight }}</span>
              </div>
              <code class="process-code">{{ step.tool }}</code>
            </div>
          </article>
        </div>
      </details>
...
async function runMatch() {
  ...
  const normalizedDirection = targetDirection.value.trim()
  const fingerprint = `${selectedResumeId.value}:${normalizedDirection}`
  if (runInputFingerprint.value !== fingerprint || !activeRunId.value) {
    activeRunId.value = crypto.randomUUID()
    runInputFingerprint.value = fingerprint
  }
  try {
    const data = await matchPositions({
      resume_id: selectedResumeId.value,
      target_direction: normalizedDirection || undefined,
      run_id: activeRunId.value,
    })
    activeRunId.value = data.run_id || activeRunId.value
...
.form-item input:focus { border-color: var(--color-primary); outline: none; box-shadow: 0 0 0 3px var(--color-focus); }
...
.resume-card { padding: 14px; border: 1px solid var(--color-border); border-radius: var(--radius-inner); cursor: pointer; transition: border-color 160ms var(--ease-out), background-color 160ms var(--ease-out), transform 130ms var(--ease-out); position: relative; }
.resume-card:hover:not(.disabled) { border-color: var(--color-border-strong); }
.resume-card.active { border-color: var(--color-primary); background: var(--color-primary-soft); }
...
.status-tag.pending { background: var(--color-primary-soft); color: var(--color-primary); }

.form-actions { display: flex; justify-content: center; margin-top: 20px; }
.btn-primary { background: var(--color-primary); color: white; border: 1px solid var(--color-primary); border-radius: var(--radius-control); padding: 10px 24px; font-size: 14px; cursor: pointer; transition: background-color 160ms var(--ease-out), border-color 160ms var(--ease-out), transform 130ms var(--ease-out); }
.btn-primary:hover:not(:disabled) { background: var(--color-primary-hover); border-color: var(--color-primary-hover); }
.btn-primary:disabled { background: #e2e8f0; color: var(--color-muted); border-color: #cbd5e1; cursor: not-allowed; transform: none; }
...
.dot-active { background: var(--color-primary); animation: pulse 1.4s infinite; }
...
.level-badge { background: var(--color-primary-soft); color: var(--color-primary); padding: 4px 12px; border-radius: 999px; font-weight: 700; font-size: 13px; }
...
.tag-blue { background: var(--color-success-soft); color: var(--color-success); }
.tag-orange { background: var(--color-warning-soft); color: var(--color-warning); }
...
.rank { background: var(--color-primary); color: white; padding: 2px 10px; border-radius: 999px; font-size: 12px; font-weight: 700; }
...
.match-bar-fill { background: linear-gradient(90deg, var(--color-primary), #34d399); height: 100%; transition: width 0.6s; }
...
.position-actions { display: flex; justify-content: flex-end; align-items: center; gap: 8px; margin-top: 10px; }
.position-jobs-link { display: inline-flex; align-items: center; min-height: 40px; }

.focus-position { font-size: 15px; font-weight: 600; margin-bottom: 10px; color: var(--color-primary); }
...
.process-card summary { display: flex; align-items: center; gap: 10px; padding: 18px 20px; color: #0f172a; cursor: pointer; list-style: none; }
.process-card summary::-webkit-details-marker { display: none; }
.process-card summary::before { content: ''; width: 0; height: 0; border-top: 5px solid transparent; border-bottom: 5px solid transparent; border-left: 6px solid var(--color-primary); transition: transform 160ms var(--ease-out); }
.process-card[open] summary::before { transform: rotate(90deg); }
.process-summary-title { font-size: 16px; font-weight: 700; }
.process-summary-meta { margin-left: auto; color: #64748b; font-size: 13px; }
.process-list { padding: 2px 20px 20px; }
.process-step { display: grid; grid-template-columns: 32px minmax(0, 1fr); gap: 12px; padding: 15px 0; border-top: 1px solid #e2e8f0; }
.process-index { width: 28px; height: 28px; display: grid; place-items: center; border-radius: 50%; background: var(--color-primary-soft); color: var(--color-primary); font-size: 12px; font-weight: 800; }
.process-content { min-width: 0; }
.process-title-row { display: flex; align-items: center; gap: 10px; }
.process-title-row h3 { font-size: 14px; color: #0f172a; }
.process-status { margin-left: auto; color: #047857; background: #ecfdf5; border: 1px solid rgba(4, 120, 87, 0.16); border-radius: 999px; padding: 3px 8px; font-size: 12px; font-weight: 700; white-space: nowrap; }
.process-content p { margin: 5px 0 8px; color: #475569; font-size: 14px; line-height: 1.6; }
.process-highlights { display: flex; flex-wrap: wrap; gap: 6px; }
.process-highlights span { padding: 4px 8px; border-radius: 6px; background: #f0fdf4; color: #166534; font-size: 12px; }
.process-code { display: inline-block; margin-top: 8px; color: #64748b; background: transparent; padding: 0; font-size: 11px; }
@media (max-width: 640px) { .process-summary-meta { display: none; } .process-list { padding: 2px 14px 14px; } .process-card summary { padding: 16px 14px; } }
code { background: var(--color-primary-soft); padding: 1px 6px; border-radius: 3px; color: var(--color-primary); font-size: 12px; }
</style>
