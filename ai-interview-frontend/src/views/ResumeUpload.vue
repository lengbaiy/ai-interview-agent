<template>
  <div class="container">
    <div :class="['card', 'resume-card', { 'resume-card-wide': step === 2 }]">
      <h2 style="margin-bottom:20px">上传简历并开始面试</h2>
      ...
      <div v-if="step === 2">
        <div class="parsed-info card">
          <h3 style="margin-bottom:12px">简历解析完成</h3>
          <div v-if="parsedContent" class="parsed-grid">
            <p v-if="parsedContent.name" class="parsed-item"><strong>姓名</strong><span>{{ parsedContent.name }}</span></p>
            <p v-if="parsedContent.education" class="parsed-item parsed-item-wide"><strong>学历</strong><span>{{ parsedContent.education }}</span></p>
            <p v-if="parsedContent.skills?.length" class="parsed-item parsed-item-wide"><strong>技能</strong><span>{{ parsedContent.skills.join('、') }}</span></p>
            <p v-if="parsedContent.summary" class="parsed-item parsed-item-wide"><strong>概述</strong><span>{{ parsedContent.summary }}</span></p>
          </div>
        </div>
        ...
        <div v-if="analysis" class="card analysis-card" style="margin-bottom:20px">
          ...
          <div class="analysis-two-col">
            ...
            <div v-if="analysis.weaknesses?.length">
              <h4 style="color:var(--color-danger);margin-bottom:8px">不足</h4>
      ...
      <div v-if="step === 'starting'" class="parsing-stage">
        <div class="ai-face-anim">
          <svg viewBox="0 0 80 80" width="80" height="80">
            <circle cx="40" cy="40" r="38" fill="white" stroke="#059669" stroke-width="2"/>
...
.resume-card {
  max-width: 600px;
  margin: 40px auto;
  transition: max-width 220ms var(--ease-out);
}
...
.upload-area {
  border: 2px dashed #cbd5e1; border-radius: var(--radius-inner); padding: 40px;
  text-align: center; cursor: pointer; transition: border-color 160ms var(--ease-out), background-color 160ms var(--ease-out);
}
...
.parsing-stage h3 { font-size: 16px; color: var(--color-foreground); margin-bottom: 20px; }
...
.progress-text { font-size: 13px; color: var(--color-muted); margin-bottom: 24px; }
.tips-area {
  min-height: 50px; display: flex; align-items: center; justify-content: center;
  background: rgba(15, 23, 42, 0.035); border-radius: var(--radius-inner); padding: 14px 20px;
}
.tip-text { font-size: 13px; color: #4b5563; line-height: 1.6; margin: 0; }
.tip-fade-enter-active, .tip-fade-leave-active { transition: opacity 0.22s var(--ease-out), transform 0.22s var(--ease-out); }
.tip-fade-enter-from { opacity: 0; transform: translateY(8px); }
.tip-fade-leave-to { opacity: 0; transform: translateY(-8px); }

/* 分析报告 */
.analysis-card { border: 1px solid var(--color-border); }
