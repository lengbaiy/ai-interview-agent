...
class AIService:
    ...
    async def _chat(messages: list, temperature: float = 0.7) -> str:
        ...
        try:
            ...
            LLM_REQUESTS.labels(operation="chat", status="success").inc()
            return content
        except Exception as e:
            LLM_REQUESTS.labels(operation="chat", status="error").inc()
            logger.error(f"DeepSeek API 调用失败: {e}")
            raise
        finally:
            LLM_LATENCY.labels(operation="chat").observe(time.perf_counter() - started)
    ...
    async def _chat_stream(messages: list, temperature: float = 0.7):
        ...
        try:
            ...
            LLM_REQUESTS.labels(operation="chat_stream", status="success").inc()
        except Exception as e:
            LLM_REQUESTS.labels(operation="chat_stream", status="error").inc()
            logger.error(f"DeepSeek 流式 API 调用失败: {e}")
            raise
        finally:
            LLM_LATENCY.labels(operation="chat_stream").observe(time.perf_counter() - started)
    ...
    async def _chat_json(
        ...
        """Call the model, validate JSON, repair once, then return a safe fallback."""
        try:
            raw = await AIService._chat(messages, temperature=temperature)
        except Exception as exc:
            logger.error("AI 调用失败，使用结构化降级结果: %s", exc)
            return model_type.model_validate(fallback)
        try:
            parsed = AIService._extract_json(raw)
            return model_type.model_validate(parsed)
        except (ValueError, TypeError, ValidationError) as first_error:
            logger.warning("AI 结构化输出校验失败，尝试修复: %s", first_error)
            repair_messages = [
                *messages,
                {
                    "role": "user",
                    "content": (
                        "上一条输出无法通过结构化校验。只返回合法 JSON，不要 markdown、解释文字或额外字段。"
                        f"原始输出：{raw[:4000]}"
                    ),
                },
            ]
            try:
                repaired = await AIService._chat(repair_messages, temperature=0.0)
                return model_type.model_validate(AIService._extract_json(repaired))
            except (ValueError, TypeError, ValidationError, Exception) as second_error:
                logger.error("AI 结构化输出修复失败，使用降级结果: %s", second_error)
                return model_type.model_validate(fallback)

    @staticmethod
    def _dump_questions(value: list[InterviewQuestion]) -> list[dict]:
        return [model_dump_compat(item) for item in value]

    @staticmethod
    def parse_answer_evaluation(text: str) -> AnswerEvaluation:
        """Parse the score JSON from a normal or streaming model response."""
        try:
            value = AIService._extract_json(text)
            evaluation = AnswerEvaluation.model_validate(value)
            feedback = re.sub(
                r"```json\s*[\s\S]*?```|\{[\s\S]*?\"score\"[\s\S]*?\}",
                "",
                text,
                flags=re.IGNORECASE,
            ).strip()
            if not evaluation.feedback:
                evaluation.feedback = feedback[:200]
            return evaluation
        except (ValueError, TypeError, ValidationError) as exc:
            logger.warning("流式回答评分校验失败，使用默认评分: %s", exc)
            return AnswerEvaluation(feedback=text.strip()[:200])
    ...
    async def parse_resume(resume_text: str) -> dict:
        ...
        result = await AIService._chat_json(
            messages,
            ResumeParseResult,
            ResumeParseResult().model_dump(),
            temperature=0.3,
        )
    ...
    async def analyze_resume(parsed_resume: dict, target_position: str) -> dict:
        ...
        result = await AIService._chat_json(
            messages,
            ResumeAnalysisResult,
            ResumeAnalysisResult().model_dump(),
            temperature=0.4,
        )
        return model_dump_compat(result)
    ...
    async def generate_questions(
        ...
        fallback = [
            InterviewQuestion(index=i, question="", category="technical").model_dump()
            for i in range(count)
        ]
        try:
            raw = await AIService._chat(messages, temperature=0.7)
            parsed = AIService._extract_json(raw)
            if not isinstance(parsed, list):
                raise ValueError("面试题输出不是数组")
            questions = [InterviewQuestion.model_validate(item) for item in parsed[:count]]
            return AIService._dump_questions(questions)
        except Exception as first_error:
            logger.warning("面试题结构化输出失败，尝试修复: %s", first_error)
            try:
                repaired = await AIService._chat(
                    [
                        *messages,
                        {
                            "role": "user",
                            "content": "请将上一条结果修复为合法 JSON 数组，每项必须含 index、question、category。",
                        },
                    ],
                    temperature=0.0,
                )
                parsed = AIService._extract_json(repaired)
                questions = [
                    InterviewQuestion.model_validate(item) for item in parsed[:count]
                ]
                return AIService._dump_questions(questions)
            except Exception as second_error:
                logger.error("面试题修复失败，使用降级结果: %s", second_error)
                return fallback
    ...
    async def select_and_adapt_questions(
        ...
        try:
            ...
            parsed = AIService._extract_json(raw)
            if not isinstance(parsed, list):
                raise ValueError("题库选题输出不是数组")
            return AIService._dump_questions(
                [InterviewQuestion.model_validate(item) for item in parsed[:target_n]]
            )
        except Exception as exc:
            logger.error("题库选题校验失败，使用召回结果降级: %s", exc)
            fallback = []
            for index, item in enumerate(candidates[:target_n]):
                fallback.append(
                    InterviewQuestion(
                        index=index,
            ...
            return AIService._dump_questions(fallback)
    ...
    async def generate_with_seeds(
        ...
        try:
            raw = await AIService._chat(messages, temperature=0.5)
            parsed = AIService._extract_json(raw)
            if not isinstance(parsed, list):
                raise ValueError("兜底出题输出不是数组")
            return AIService._dump_questions(
                [InterviewQuestion.model_validate(item) for item in parsed[:target_n]]
            )
        except Exception as exc:
            logger.error("兜底出题校验失败，使用种子题降级: %s", exc)
            fallback = []
            for index, item in enumerate(seed_questions[:target_n]):
                fallback.append(
                    InterviewQuestion(
                        index=index,
                        question=item.get("question", ""),
                        category=item.get("category", "technical"),
                        bank_id=item.get("id") or item.get("bank_id"),
                        reference_answer=item.get("reference_answer"),
                        key_points=item.get("key_points"),
                        source=item.get("source", "from_bank"),
                    )
                )
            while len(fallback) < target_n:
                fallback.append(
                    InterviewQuestion(
                        index=len(fallback),
                        question="请结合你的项目经历说明一次具体的技术决策。",
                        category="project",
                        source="ai_fallback",
                    )
                )
