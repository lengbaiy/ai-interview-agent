...
class KnowledgeService:
    ...
    async def retrieve_chunks(
        ...
        """
        混合检索文档 chunk，默认使用配置中的 semantic/keyword/hybrid/advanced 模式。
        """
        ...
        try:
            if mode == "advanced":
                ...
                return await AdvancedRAGService.retrieve_knowledge(
                    query=query,
                    k=k,
                    category=category,
                    min_score=min_score,
            ...
            if mode in {"semantic", "hybrid"}:
                semantic = await KnowledgeService._semantic_search(
                    query, db, k, category, min_score
                )
            if mode in {"keyword", "hybrid"}:
                keyword = await KnowledgeService._keyword_search(query, db, k, category)
            if mode == "semantic":
                return semantic
            if mode == "keyword":
                return keyword
            ...
            RAG_REQUESTS.labels(retriever="knowledge", mode=mode).inc()
            RAG_LATENCY.labels(retriever="knowledge", mode=mode).observe(
    ...
    async def _keyword_search(
        ...
        document = func.to_tsvector("simple", func.coalesce(KnowledgeChunk.content, ""))
