...
class InterviewService:
    ...
    async def _generate_questions_with_rag(
        ...
        if cnt >= total_questions:
            ...
            logger.warning("[RAG出题] 题库为空，走【纯 AI 生成】兜底分支")
        ...
        if is_finished:
            ...
            try:
                ...
                if settings.AGENT_MEMORY_ENABLED:
                    ...
                    await agent_runtime.memory.save_interview_report(
                        user_id,
                        report,
    ...
    async def submit_answer_stream(
        ...
        # 从流式文本中提取评分，统一走 Pydantic 校验并保留降级结果
        evaluation = AIService.parse_answer_evaluation(full_text)
        score = evaluation.score
        feedback = evaluation.feedback or full_text.strip()
