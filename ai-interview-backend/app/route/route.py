...
def create_app():
    ...
    async def metrics(x_metrics_token: str | None = Header(default=None)):
        ...
        if settings.ENV == "production":
            if not settings.METRICS_AUTH_TOKEN or not secrets.compare_digest(
                x_metrics_token or "", settings.METRICS_AUTH_TOKEN
            ):
                return Response(status_code=404)
    ...
    # Only avatars are public; resumes remain protected by their API ownership checks.
