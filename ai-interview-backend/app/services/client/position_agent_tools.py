...
async def match_positions(
    ...
    if categories:
        ...
        templates = [item for item in templates if item.category.lower() in allowed]

        # The candidate profile may contain a category for which operations have
        # not configured a template yet. Keep matching available in that case.
        if not templates:
            logger.warning(
                "No position templates for categories %s; falling back to all active templates",
                sorted(allowed),
            )
