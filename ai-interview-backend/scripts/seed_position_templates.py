...
async def main():
    """修复已知历史模板，并补齐缺失的标准岗位模板。"""
    AsyncSessionLocal = get_session_local()
    async with AsyncSessionLocal() as db:
        inserted = 0
        skipped = 0
        repaired = 0
        seed_by_tag = {item["position_tag"]: item for item in SEED_POSITIONS}

        # Early development data used ``ai`` as a tag and incorrectly assigned
        # it to backend. Convert only that known legacy record so existing
        # deployments retain their primary key and manager audit trail.
        legacy_ai = (await db.execute(
            select(PositionTemplate).where(PositionTemplate.position_tag == "ai")
        )).scalar_one_or_none()
        if legacy_ai and not (await db.execute(
            select(PositionTemplate).where(PositionTemplate.position_tag == "ai_application")
        )).scalar_one_or_none():
            for field, value in seed_by_tag["ai_application"].items():
                setattr(legacy_ai, field, value)
            repaired += 1
            print("  [修复] ai -> ai_application（AI 应用开发工程师）")
        ...
        for item in SEED_POSITIONS:
            ...
            inserted += 1
        await db.commit()
        print(f"\n种子数据完成：修复 {repaired} 条，新增 {inserted} 条，跳过 {skipped} 条")
