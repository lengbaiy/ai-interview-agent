  ...
  <div>
    ...
    <div class="card filter-bar">
      ...
      <select v-model="filter.position_tag" @change="reload">
        ...
        <option value="ai_application">AI 应用开发</option>
        <option value="fullstack">全栈</option>
        <option value="mobile_android">Android 开发</option>
    ...
    <div class="card table-card">
      ...
      <table class="question-table">
        <colgroup>
          ...
          <col class="col-use-count" />
          <col class="col-embedding" />
        ...
        <tbody>
          <tr v-for="q in items" :key="q.id">
            <td>{{ q.id }}</td>
            <td><span class="badge badge-blue">{{ categoryLabel(q.category) }}</span></td>
            <td><span class="position-label">{{ positionLabel(q.position_tag) }}</span></td>
            <td><span :class="['badge', diffClass(q.difficulty)]">{{ difficultyLabel(q.difficulty) }}</span></td>
            <td class="cell-question">
              <div class="question-text">{{ q.question }}</div>
              <a v-if="q.source_url" class="source-link" :href="q.source_url" target="_blank" rel="noopener noreferrer">
                {{ sourceLabel(q.source) }} 原题
              </a>
              <span v-else-if="q.source !== 'manual'" class="source-text">{{ sourceLabel(q.source) }}</span>
            </td>
            <td>{{ q.use_count }}</td>
            <td>
...
const categoryLabels = { technical: '技术基础', behavioral: '行为面试', system_design: '系统设计', project: '项目经历' }
const positionLabels = {
  python_backend: 'Python 后端', java_backend: 'Java 后端', vue_frontend: 'Vue 前端',
  react_frontend: 'React 前端', ai_application: 'AI 应用开发', fullstack: '全栈开发',
  mobile_android: 'Android 开发', devops: 'DevOps / 运维', algorithm: '算法基础'
}
const difficultyLabels = { easy: '简单', medium: '中等', hard: '困难' }
const sourceLabels = { manual: '手工录入', imported: '批量导入', ai_generated: 'AI 生成', codeforces: 'Codeforces' }
function categoryLabel(value) { return categoryLabels[value] || value }
function positionLabel(value) { return positionLabels[value] || value }
...
.col-position { width: 138px; }
.col-difficulty { width: 84px; }
.col-question { width: auto; }
.col-use-count { width: 104px; }
.col-embedding { width: 92px; }
.col-status { width: 86px; }
.col-actions { width: 200px; }

.cell-question { max-width: 0; overflow: hidden; }
.question-text { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.source-link, .source-text { display: inline-block; margin-top: 4px; font-size: 12px; color: var(--color-primary); white-space: nowrap; }
.source-text { color: #64748b; }
.position-label { color: #334155; font-weight: 500; }
